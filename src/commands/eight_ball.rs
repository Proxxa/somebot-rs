...
#[export_name = "8ball"]
pub async fn _8ball(ctx: Context<'_>, #[description = "The question supplied to the bot. (Does nothing)"] question: String) -> Result<(), Error> {
    ...
    ctx.send(|reply| reply.embed(|embed| embed
        .title(question)
        .description(message)
        .color(embed_color)
    ))
